...
class sliders {
    constructor(parent, mode, id) {
        this.hslSetUp = () => {
            ...
            this.ranges = {
                hue: this._createSlider(this.id, defaults[0][0], defaults[0][1], defaults[0][2], defaults[0][3]),
                saturation: this._createSlider(this.id, defaults[1][0], defaults[1][1], defaults[1][2]),
                lightness: this._createSlider(this.id, defaults[2][0], defaults[2][1], defaults[2][2]),
            };
        };
        this._createSlider = (parentId, defaultSliderValue, max, label, suggestedVals, min, step) => {
            ...
            if (suggestedVals && suggestedVals.length > 0) {
                ...
                input.setAttribute("list", `colorPicker-slider-DataList-${parentId}-${label}`);
            ...
            openLockSVGElement.addEventListener("click", (event) => {
                ...
                rangeWrap.dispatchEvent(cu.createColorEvent("lock", {
                    type: label,
            ...
            const resetSVGElement = LabelAndButtonWrapper.lastChild;
            resetSVGElement.addEventListener("click", () => {
                rangeWrap.dispatchEvent(cu.createColorEvent("reset", {
                    type: label,
                    value: defaultSliderValue,
                }, { reset: true }));
            });
            input.addEventListener("input", (event) => {
                rangeWrap.dispatchEvent(cu.createColorEvent("slider", {
                    type: label,
                    value: parseInt(event.currentTarget.value),
                }));
            });
        ...
        this.colorChangeSubtype = (source, change) => {
    ...
    colorChange(changeEvent) {
        if (changeEvent.change.type !== "hsl") {
            ...
            cu.colorPicker_colorToListOfChangeEvents(changeEvent.change.value).forEach((element) => {
